...
- Runs only when `verbose`. Logs the per-term energy
  `bond=.. angle=.. chiral=.. plane=.. cistrans=.. vdw=.. distance=.. rmsd=..
...
restraints_config:
  ...
  conformer_restraints_config:
    ...
    plane: {weight: 1.0}                              # best-fit-plane flatness of whole planar groups (out-of-plane RMS deviation ~0). OFF by default (weight 0); fires on aromatic/conjugated rings AND non-ring sp2 groups (carbonyl/amide/ester/carboxyl/trisubstituted), membership confirmed by reference-conformer coplanarity. optional slack (Angstrom).
