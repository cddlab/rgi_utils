"""PyTorch restraint energies backed by the shared geometry kernels."""
...
def _plane_normal(covariance):
    covariance = _OPS.stop_gradient(covariance)
    _values, vectors = _OPS.eigh(covariance)
    return _OPS.stop_gradient(vectors[..., :, 0])
...
def prepare_spec(spec, device="cpu", dtype=torch.float32):
    """Convert a backend-agnostic ``RestraintSpec`` into torch tensors."""
    ...
    return pack_spec(spec, to_int, to_float)
