...
def make_minimizer(
    ...
    # custom (registry) restraints are per-entry CG energy terms too (gated inside
    # total_energy via sigma), so the solver branch must run when any is present.
    has_registered = spec.has_registered()
    ...
    def _descend(coords, sigma):
        ...
        if has_conf or has_rmsd or has_vdw or has_group or has_registered:
            if has_vdw:
