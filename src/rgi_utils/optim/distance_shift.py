...
This is exactly the configuration the per-atom CG used to converge to (the
distance gradient is uniform within a group, so CG just rigid-translates), but in
one O(N) step instead of ~max_iter iterations. The three backends share the maths;
only the gather/scatter primitives differ. ``prepared`` is the ``distance`` dict
that ``energy.*_energy.prepare_spec`` already builds (grp1_idx/grp2_idx local
indices, grp1_mask/grp2_mask, target1/target2, dist_type, mask, start_sigma).
"""
...
def apply_distance_shift_numpy(active, d, sigma=None):
    ...
    delta = np.where(dt == 0, dh, np.where(dt == 1, df, np.where(dt == 2, dl, du)))
    gate = d["mask"].astype(np.float64)
    if sigma is not None:
        gate = gate * (float(sigma) <= d["start_sigma"]).astype(np.float64)
    delta = delta * gate
    denom = n1 + n2 + _EPS
    com1_shift = (-delta * (n2 / denom))[..., None] * u  # (..., D, 3)
    com2_shift = (delta * (n1 / denom))[..., None] * u
    pa1 = (com1_shift[..., :, None, :] * m1[..., None]).reshape(*com1_shift.shape[:-2], -1, 3)
    pa2 = (com2_shift[..., :, None, :] * m2[..., None]).reshape(*com2_shift.shape[:-2], -1, 3)
...
def apply_distance_shift_torch(active, d, sigma=None):
    ...
    gate = d["mask"].to(dist.dtype)
    if sigma is not None:
        gate = gate * (float(sigma) <= d["start_sigma"]).to(dist.dtype)
    delta = delta * gate
    denom = n1 + n2 + _EPS
    ...
    active = active.index_add(-2, idx1.reshape(-1), pa1.to(active.dtype))
    active = active.index_add(-2, idx2.reshape(-1), pa2.to(active.dtype))
    return active
...
def apply_distance_shift_jax(active, d, sigma):
    ...
    delta = jnp.where(dt == 0, dh, jnp.where(dt == 1, df, jnp.where(dt == 2, dl, du)))
    gate = d["mask"]
    if sigma is not None:
    ...
    active = active.at[..., idx1.reshape(-1), :].add(pa1)
    active = active.at[..., idx2.reshape(-1), :].add(pa2)
    return active
